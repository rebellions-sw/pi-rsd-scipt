...
data_loging(){
    ...
    while ps -p $pid > /dev/null; do
		...
		GDR_VDDQ_V=$(echo "${rbln_pmic}" | awk '$1 == "GDR" && $2 == "VDDQ" && NR==1{print $4}')
		GDR_VDDQ_A=$(echo "${rbln_pmic}" | awk '$1 == "GDR" && $2 == "VDDQ" && NR==1{print $7}')
                GDR_VDDQ_W=$(echo "${rbln_pmic}" | awk '$1 == "GDR" && $2 == "VDDQ" && NR==1{print $10}')
	...
	sleep 0.1
        echo "$base_file_name $IDS Waiting for start_test function to finish..."
    done
}
...
analysis_log(){
	...
	total_time=$(grep 'waited functions' $log_file | awk '{if ($8 != "0.00") print $8}')
        echo "test_result = $test_result   /  waited_avg_time = $waited_avg_time / called_avg_time = $called_avg_time / total_time = $total_time"
        echo "No,waited,called,report_passed,average_us,total_time" > $cvs_file

        awk -v report_passed="$test_result" -v average_us="$waited_avg_time" -v called_us="$called_avg_time" -v total_time="$total_time" '
            /Perf \(waited inferences\)/ {
                no = $6
                waited_perf = $8
                getline
                called_perf = $8
                print no "," waited_perf "," called_perf "," report_passed "," average_us "," total_time
            }
        ' $log_file >> $cvs_file
...
start_mda_test() {
    ...
    local file_name=$2
    base_file_name=$(basename "$file_name")	
    echo "RBLNTHUNK_PERF=2 rblntrace retrace $NPU_ID_LIST $file_name"
    log_file=${log_path}/${host_name}_${test_num}_execution_log_${LOG_SDATE}_${base_file_name}_${NPU_ID_LIST}.log
